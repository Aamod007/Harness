...
mod tests {
    ...
    fn folds_subdirectories_into_repo_root() {
        ...
        let locs = vec![
            SessionLocation::new("/u/Aamod/proj", Some(ts(2026, 6, 24))),
            SessionLocation::new("/u/Aamod/proj/crates/a", Some(ts(2026, 6, 24))),
            SessionLocation::new("/u/Aamod/proj/crates/b", Some(ts(2026, 6, 23))),
        ];
        let ranked = rank_repositories_with(
            &locs,
            now,
            &RankOptions::default(),
            fake_resolver(&["/u/Aamod/proj"]),
        );
        assert_eq!(ranked.len(), 1);
        assert_eq!(ranked[0].path, "/u/Aamod/proj");
    ...
    fn excluded_paths_are_dropped() {
        let now = ts(2026, 6, 25);
        let locs = vec![
            SessionLocation::new("/home/Aamod", Some(ts(2026, 6, 24))),
            SessionLocation::new("/home/Aamod/work", Some(ts(2026, 6, 24))),
        ];
        let opts = RankOptions {
            excluded_paths: vec![PathBuf::from("/home/Aamod")],
            ..RankOptions::default()
        };
        let ranked = rank_repositories_with(
            &locs,
            now,
            &opts,
            fake_resolver(&["/home/Aamod", "/home/Aamod/work"]),
        );
        assert_eq!(ranked.len(), 1);
        assert_eq!(ranked[0].path, "/home/Aamod/work");
    }
    ...
    fn plan_assigns_top_repo_home_then_next_repos() {
        let ranked = vec![
            repo("/u/Aamod/jcode", 600.0),
            repo("/u/Aamod/scrollwm", 100.0),
            repo("/u/Aamod/sideproj", 50.0),
            repo("/u/Aamod/fourth", 10.0),
            repo("/u/Aamod/fifth", 5.0),
        ];
        ...
        assert_eq!(plan[0].dir, "/u/Aamod/jcode");
        assert_eq!(plan[1].chord, "cmd+'");
        assert_eq!(plan[1].dir, "/u/Aamod");
        assert_eq!(plan[1].label, "home");
        assert_eq!(plan[2].chord, "cmd+[");
        assert_eq!(plan[2].dir, "/u/Aamod/scrollwm");
        assert_eq!(plan[3].chord, "cmd+]");
        assert_eq!(plan[3].dir, "/u/Aamod/sideproj");
        assert_eq!(plan[4].chord, "cmd+\\");
        assert_eq!(plan[4].dir, "/u/Aamod/fourth");
    }

    #[test]
    fn plan_skips_home_if_it_appears_in_ranking() {
        let ranked = vec![
            repo("/u/Aamod", 999.0), // home ranked #1, should not take a repo slot
            repo("/u/Aamod/jcode", 600.0),
        ];
        let plan = build_launch_hotkey_plan(
            Path::new("/u/Aamod"),
    ...
    fn plan_truncates_to_available_repos() {
        let ranked = vec![repo("/u/Aamod/only", 600.0)];
        let plan = build_launch_hotkey_plan(
            Path::new("/u/Aamod"),
            &ranked,
            &DEFAULT_LAUNCH_HOTKEY_CHORDS,
        ...
        assert_eq!(plan[0].dir, "/u/Aamod/only");
        assert_eq!(plan[1].dir, "/u/Aamod");
    }
    ...
    fn plan_with_no_repos_still_binds_home() {
        let plan =
            build_launch_hotkey_plan(Path::new("/u/Aamod"), &[], &DEFAULT_LAUNCH_HOTKEY_CHORDS);
        assert_eq!(plan.len(), 1);
        assert_eq!(plan[0].chord, "cmd+;");
        assert_eq!(plan[0].dir, "/u/Aamod");
    ...
    fn collect_reads_working_dirs_and_skips_junk() {
        ...
        write(
            "session_a.json",
            r#"{"working_dir":"/Users/Aamod/jcode-github","id":"a"}"#,
        );
        write(
            "session_b.json",
            r#"{"working_dir":"/Users/Aamod/scrollwm","id":"b"}"#,
        );
        ...
        assert_eq!(
            dirs,
            vec!["/Users/Aamod/jcode-github", "/Users/Aamod/scrollwm"]
        );
