#!/usr/bin/env bash
# Verify the *published* @aamodkumar/jcode-sdk tarball, not just its source.
#
...
import { JcodeClient, HarnessError, API_VERSION_MAJOR } from "@aamodkumar/jcode-sdk";
...
if [ -n "$runtime_tarball" ]; then
  ...
  node --input-type=module -e '
  import { bundledJcodeBinary } from "@aamodkumar/jcode-sdk";
  const binary = bundledJcodeBinary();
  if (!binary || !binary.includes("@aamodkumar/jcode-linux-")) {
...
const sdk = require("@aamodkumar/jcode-sdk");
...
JSON
cat > consumer.ts <<'TS'
import { JcodeClient, HarnessError, type TurnResult, type ApiEvent } from "@aamodkumar/jcode-sdk";
...
cat > narrowing.ts <<'TS'
import { isKnownEvent, type AnyApiEvent, type ApiEvent } from "@aamodkumar/jcode-sdk";
...
import { JcodeClient } from "@aamodkumar/jcode-sdk";
